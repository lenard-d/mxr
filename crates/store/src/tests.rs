...
async fn upsert_envelope_with_direction_writes_direction_column() {
    ...
    store
        ...
        .await
        .unwrap();
    let direction_after: (String,) = sqlx::query_as("SELECT direction FROM messages WHERE id = ?")
        .bind(env.id.as_str())
        .fetch_one(store.reader())
        .await
        .unwrap();
...
async fn insert_body_promotes_list_id_to_messages_column() {
    ...
    let pre: Vec<(Option<String>,)> = sqlx::query_as("SELECT list_id FROM messages WHERE id = ?")
        .bind(env.id.as_str())
        .fetch_all(store.reader())
        .await
        .unwrap();
    ...
    let post: Vec<(Option<String>,)> = sqlx::query_as("SELECT list_id FROM messages WHERE id = ?")
        .bind(env.id.as_str())
        .fetch_all(store.reader())
        .await
        .unwrap();
    ...
    let post2: Vec<(Option<String>,)> = sqlx::query_as("SELECT list_id FROM messages WHERE id = ?")
        .bind(env.id.as_str())
        .fetch_all(store.reader())
        .await
        .unwrap();
...
async fn upsert_envelope_writes_direction_unknown_and_null_list_id_by_default() {
    ...
    let directions: Vec<(String,)> = sqlx::query_as("SELECT direction FROM messages ORDER BY id")
        .fetch_all(store.reader())
        .await
        .unwrap();
