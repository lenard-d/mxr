...
**Plain text first** — mxr renders text/plain bodies if they exist, falling back to HTML→text only when needed. HTML remote images are controlled by `render.html_remote_content`; with it on, the TUI and CLI fetch every image the HTML points at, tracking pixels included. The web app loads normal remote images by default and filters images that declare a width or height of two pixels or less, plus images from known tracker hosts.
